module distortion #(
   ...
   always @(posedge clk) begin
      if(rst) begin
         ...
         dist_data[0] <= i_dat * gain;
         // compare with threshold
         if(dist_data[0] > 0)
            dist_data[1] <= (dist_data[0] > THRESH) ?
               THRESH :
               dist_data[0]; // maybe consider negative gain functionality
         else
            dist_data[1] <= (dist_data[0] < (-1)*THRESH) ?
               (-1)*THRESH :
               dist_data[0]; // maybe consider negative gain functionality


         // select output based on enable
         // o_dat <= (enable) ? dist_data[1] : pure_data[1];
      end     
   end

   assign o_dat = (enable) ? dist_data[1] : pure_data[1];
